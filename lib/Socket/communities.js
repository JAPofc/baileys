...
export const makeCommunitiesSocket = (config) => {
    ...
    async function parseGroupResult(node) {
        ...
        if (groupNode) {
            try {
                ...
                logger.error({ err: error }, 'error parsing community group metadata'); // JAP@Fix: was raw console.error
    ...
    return {
        ...
        // JAP@Add: community invite code (same w:g2 shape as group invites)
        communityGetInviteCode: async (jid) => {
...
export const extractCommunityMetadata = (result) => {
    ...
    const metadata = {
        ...
        participants: getBinaryNodeChildren(community, 'participant').map(({ attrs }) => {
            // JAP@Fix (bug 59): ported PN/LID field population from groups.js — was previously TODO/unset
            return {
                id: attrs.jid,
