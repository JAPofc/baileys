...
export const makeBusinessSocket = (config) => {
    ...
    const updateCoverPhoto = async (photo) => {
        ...
        // JAP@Fix (media backend cleanup): this temp file was NEVER deleted —
        // every cover-photo update leaked one file in tmpdir forever.
        let meta_hmac, fbid, ts;
        try {
            ({ meta_hmac, fbid, ts } = await waUploadToServer(filePath, {
