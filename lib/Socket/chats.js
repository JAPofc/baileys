...
// JAP@Fix (upstream 731cd6b, post-rc14): profile picture IQ content builder —
// nests the tctoken INSIDE the <picture> node (WA Web behaviour) instead of
// appending it as a sibling, which the server ignores.
...
export const makeChatsSocket = (config) => {
    ...
    const profilePictureUrl = async (jid, type = 'image', timeoutMs = 5000, shouldIncludeTcToken = false) => {
        ...
        // JAP@Fix (upstream 731cd6b, post-rc14): the tctoken must be NESTED inside
        // the <picture> node, not appended as its sibling — WA Web nests it, and a
        // sibling tctoken is ignored by the server (privacy-gated pictures then
        // come back empty even when we hold a valid token).
        let tcTokenContent;
