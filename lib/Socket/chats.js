...
// JAP@Fix: profile picture IQ content builder — nests the tctoken INSIDE the
// <picture> node (WA Web behaviour) instead of appending it as a sibling,
...
export const makeChatsSocket = (config) => {
    ...
    // JAP@Fix (LID/PN unify): when no mapping exists, the old code did
    // jidNormalizedUser(undefined) which returns '' — an empty-string jid that
    // passes `!== undefined` checks and can leak into wire nodes. Missing
    // mappings now stay `undefined`, and both branches share one normalizer.
    const normalizeOrUndefined = (jid) => {
        const n = jid ? jidNormalizedUser(jid) : '';
        return n || undefined;
    };
    const findUserId = async (pnLid) => {
        ...
        if (isPnUser(normalizedJid) || isHostedPnUser(normalizedJid)) {
            ...
            userId.lid = normalizeOrUndefined((await signalRepository.lidMapping.getLIDsForPNs([normalizedJid]))?.[0]?.lid);
        }
        else if (isLidUser(normalizedJid) || isHostedLidUser(normalizedJid)) {
            userId.lid = normalizedJid;
            userId.phoneNumber = normalizeOrUndefined((await signalRepository.lidMapping.getPNsForLIDs([normalizedJid]))?.[0]?.pn);
        }
        else {
    ...
    const profilePictureUrl = async (jid, type = 'image', timeoutMs = 5000, shouldIncludeTcToken = false) => {
        ...
        // JAP@Fix: the tctoken must be NESTED inside the <picture> node, not
        // appended as its sibling — WA Web nests it, and a sibling tctoken is
        // ignored by the server (privacy-gated pictures then come back empty
        // even when we hold a valid token).
        let tcTokenContent;
