...
import { addTransactionCapability, aesEncryptCTR, bindWaitForConnectionUpdate, buildPairingQRData, bytesToCrockford, configureSuccessfulPairing, Curve, derivePairingCodeKey, generateLoginNode, generateMdTagPrefix, generateRegistrationNode, getCodeFromWSError, getCompanionPlatformId, getErrorCodeFromStreamError, getNextPreKeysNode, makeEventBuffer, makeNoiseHandler, promiseTimeout, renderQRToTerminal, signedKeyPair, xmppSignedPreKey } from '../Utils/index.js';
...
export const makeSocket = (config) => {
    ...
    const onWhatsApp = async (...phoneNumber) => {
        ...
        for (const jid of phoneNumber) {
            if (isLidUser(jid)) {
                // JAP@Fix (bug 7): was just warn+skip — now attempt PN lookup via LID mapping
                lidUsers.push(jid);
                continue;
            }
            else {
    ...
    if (printQRInTerminal) {
        ev.on('connection.update', ({ qr }) => {
            ...
            try {
                console.log(renderQRToTerminal(qr));
                console.log('Scan the QR above with WhatsApp (Linked devices > Link a device)');
            }
            catch (e) {
                logger.warn({ err: e }, 'failed to render QR in terminal');
            }
        });
    }
    ...
    let qrTimer;
    let closed = false;
    // JAP@Add: true once the noise handshake has completed and encrypted
    // frames can be sent safely (set on pair-device stanza / connection open).
    let pairingReadyForSend = false;
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        // JAP@Fix (pairing validation): the phone number must be digits-only
        // in E.164 form — a '+' or spaces produced a malformed jid the server
        // silently never delivered a code for. Normalize instead of failing.
        const cleanPhone = String(phoneNumber ?? '').replace(/\D/g, '');
        if (!cleanPhone || cleanPhone.length < 7) {
            throw new Boom('requestPairingCode: phoneNumber must be the full international number (digits only, e.g. 628123456789)', { statusCode: 400 });
        }
        // JAP@Fix (custom-code validation): only length was checked. Lowercase
        // input or characters outside the Crockford set (0, I, O, U are NOT
        // valid) passed through and WhatsApp rejected the pairing silently on
        // the phone. Uppercase first, then validate the real alphabet.
        let pairingCode;
        if (customPairingCode) {
            const normalized = String(customPairingCode).toUpperCase();
            if (normalized.length !== 8) {
                throw new Boom('Custom pairing code must be exactly 8 chars', { statusCode: 400 });
            }
            if (!/^[123456789ABCDEFGHJKLMNPQRSTVWXYZ]{8}$/.test(normalized)) {
                throw new Boom('Custom pairing code may only use Crockford base32 characters (1-9, A-Z excluding I, O, U)', { statusCode: 400 });
            }
        ...
        // JAP@Fix (pairing race): calling requestPairingCode() right after
        // makeWASocket() — the documented usage — raced the WebSocket/noise
        // handshake and threw 'Connection Closed' from sendNode. Note that
        // ws.isOpen alone is NOT enough: the socket opens before the noise
        // handshake completes, and a frame sent in that window goes out
        // unencrypted (encodeFrame has no transport yet) and kills the
        // connection. The first `qr` / `connection: 'open'` update is only
        // emitted after the handshake, so gate on that signal instead.
        if (!pairingReadyForSend) {
            await bindWaitForConnectionUpdate(ev)(({ connection, qr }) => connection === 'open' || !!qr, connectTimeoutMs);
        }
        authState.creds.pairingCode = pairingCode;
        authState.creds.me = {
            id: jidEncode(cleanPhone, 's.whatsapp.net'),
            name: '~'
        };
    ...
    ws.on('CB:iq,type:set,pair-device', async (stanza) => {
        pairingReadyForSend = true; // JAP@Add: stanza is post-handshake by definition
        const iq = {
            tag: 'iq',
            attrs: {
                to: S_WHATSAPP_NET,
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            // JAP@Fix (bug 37): rotateSignedPreKey was never called automatically.
        ...
        pairingReadyForSend = true; // JAP@Add: fully logged in — sends are safe
