...
import { DisconnectReason, ReachoutTimelockEnforcementType, WAMessageStatus, WAMessageStubType } from '../Types/index.js';
...
export const makeMessagesRecvSocket = (config) => {
    ...
    const requestPlaceholderResend = async (messageKey, msgData) => {
        ...
        setTimeout(async () => {
            // JAP@Fix: a rejecting cache (redis down, etc.) used to surface as
            // an unhandledRejection from this fire-and-forget timer.
            try {
                if (await placeholderResendCache.get(messageKey?.id)) {
                ...
                logger.warn({ err }, 'placeholder-resend cache cleanup failed');
            }
        }, 8000);
        return sendPeerDataOperationMessage(pdoMessage);
    };
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'picture':
                ...
                // JAP@Fix (bug 53): implement proper WAJIDHASH support for picture updates
    ...
    const sendMessagesAgain = async (key, ids, retryNode, receiptNode) => {
        ...
        // JAP@Fix (session recovery): the retry receipt's Signal error code was
        // parsed nowhere — MessageRetryManager.parseRetryErrorCode/isMacError
        // existed as dead code. A peer reporting Bad MAC (codes 4/7) means the
        // session is provably out of sync, yet we only recreated it after
        // retryCount > 1 AND an hour since the last recreation. Now the error
        // code is extracted and MAC errors recreate the session immediately,
        // even on the first retry.
        // wire format: <receipt type="retry"><retry .../><error code="7"/>…</receipt>
        const errorNode = getBinaryNodeChild(receiptNode, 'error');
        const retryErrorCode = messageRetryManager
            ? messageRetryManager.parseRetryErrorCode(errorNode?.attrs?.code ?? retryNode?.attrs?.error)
            : undefined;
        const macFailure = messageRetryManager?.isMacError(retryErrorCode) ?? false;
        let shouldRecreateSession = false;
        let recreateReason = '';
        if (enableAutoSessionRecreation && messageRetryManager && (retryCount > 1 || macFailure) && !injectedFromBundle) {
            try {
                ...
                recreateReason = result.reason;
                if (shouldRecreateSession) {
                    logger.debug({ participant, retryCount, retryErrorCode, reason: recreateReason }, 'recreating session for outgoing retry');
                    await authState.keys.set({ session: { [sessionId]: null } });
                }
            }
            catch (error) {
    ...
    // JAP@Add (delivery tracking) --------------------------------------------
    // waitForMessageAck(messageId): resolves when the server acks the message,
    // rejects when the ack carries an error code or the timeout elapses.
    // Use together with sendMessage for delivery-critical flows:
    //   const msg = await sock.sendMessage(jid, { text })
    //   await sock.waitForMessageAck(msg.key.id) // server accepted it
    const pendingMessageAcks = new Map();
    const waitForMessageAck = (messageId, timeoutMs = 60_000) => {
        if (!messageId) {
        ...
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                ...
                reject(new Boom('Timed out waiting for server ack', { statusCode: DisconnectReason.timedOut, data: { messageId } }));
            }, timeoutMs);
            timer.unref?.();
            pendingMessageAcks.set(messageId, { resolve, reject, timer });
        });
    };
    // reject all waiters when the socket dies so callers never hang
    registerSocketEndHandler?.(() => {
        for (const [id, waiter] of pendingMessageAcks) {
            clearTimeout(waiter.timer);
            waiter.reject(new Boom('Connection closed before the server acked the message', { statusCode: DisconnectReason.connectionClosed, data: { messageId: id } }));
        }
        pendingMessageAcks.clear();
    });
    const handleBadAck = async ({ attrs }) => {
        const key = { remoteJid: attrs.from, fromMe: true, id: attrs.id };
        // JAP@Fix (bug 54/55): implement phash handling with resend logic
    ...
    ws.on('CB:ack,class:message', (node) => {
        // JAP@Add (delivery tracking): settle any waitForMessageAck() waiters.
        // Every outgoing message gets exactly one server ack node — error attr
        // present means the server refused it, absence means it was accepted.
        const ackId = node.attrs?.id;
        if (ackId && pendingMessageAcks.has(ackId)) {
            const waiter = pendingMessageAcks.get(ackId);
            pendingMessageAcks.delete(ackId);
            clearTimeout(waiter.timer);
            if (node.attrs.error) {
                waiter.reject(new Boom(`Message was not accepted by the server (error ${node.attrs.error})`, {
                    statusCode: +node.attrs.error || 500,
                    data: node.attrs
                }));
            }
            else {
                waiter.resolve({ messageId: ackId, from: node.attrs.from, timestamp: node.attrs.t ? +node.attrs.t : undefined });
            }
        }
