...
const getFfmpeg = async () => {
    if (_ffmpeg === undefined) {
        ...
        // JAP@Add --- point fluent-ffmpeg at the auto-detected binary
        // (ffmpeg-static / @ffmpeg-installer / FFMPEG_PATH); with no match it
        // keeps fluent-ffmpeg's own PATH lookup as before.
        if (_ffmpeg) {
            try {
                const { resolveFfmpegPath } = await import('../Utils/ffmpeg-path.js');
                const bin = await resolveFfmpegPath();
                if (bin && bin !== 'ffmpeg')
                    _ffmpeg.setFfmpegPath(bin);
            }
            catch { /* keep default PATH lookup */ }
        }
    }
    if (!_ffmpeg)
...
export class MediaManager {
    /**
     * JAP@Fix (FFmpeg limits): every conversion used to await an unbounded
     * ffmpeg run — a corrupt/adversarial input that makes ffmpeg hang (or
     * decode-bomb) stalled the promise forever and left a zombie process.
     * Conversions now enforce a wall-clock timeout (default 60s, override via
     * MediaManager.ffmpegTimeoutMs) and an input size cap (default 64 MB,
     * override via MediaManager.maxInputBytes; 0 disables). On timeout the
     * ffmpeg process is SIGKILLed and the promise rejects.
     */
    ...
    static _runFfmpeg(command) {
        return new Promise((resolve, reject) => {
            let timer = null;
            const done = (fn) => (arg) => { if (timer) { clearTimeout(timer); timer = null; } fn(arg); };
            ...
            command.on('end', done(() => resolve())).on('error', done((err) => reject(err))).run();
        });
    }
    ...
    static async convertToSticker(inputPathOrBuffer, metadata) {
        ...
        try {
            ...
            await MediaManager._runFfmpeg(ffmpegLib(tempInput)
                .outputOptions([
                '-vcodec', 'libwebp',
