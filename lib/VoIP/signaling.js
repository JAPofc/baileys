...
export class SignalingBridge {
  ...
  // JAP@Add --- ourin-baileys's socket exposed a `getPrivacyTokens(jids)` helper for
  // fetching `trusted_contact` tokens; the base socket here doesn't (it only has
  // `issuePrivacyTokens`, which *issues* tokens rather than *requesting* someone else's).
  // Self-contained so this module doesn't require patching Socket/messages-send.js.
  ...
  #doProcessIncomingCall = async (node, voip, activeCallId) => {
    ...
    switch (usableNode.tag) {
      case "offer":
        ...
        // JAP@Add --- surface inbound offers so bots can react (log / auto-reply).
        // NOTE: ANSWERING inbound calls is NOT supported — the WASM build only
        // exposes outbound startVoipCall/endCall, with no accept path.
        try {
          this.#onIncomingCall?.({
