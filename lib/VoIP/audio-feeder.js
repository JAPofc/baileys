...
export class AudioFeeder {
    ...
    #started = false; // JAP@Add --- start() called and not yet stop()ed (survives decoder death)
    ...
    /**
     * JAP@Add --- swap the audio source mid-stream (IVR-style: greeting →
     * menu → hold music). Kills the current ffmpeg decoder, drops any queued
     * audio from the old source, and starts decoding the new one; the uplink
     * cadence resumes after the usual warm-up. If the feeder has not been
     * started yet, the new source is simply used on the next start().
     */
    swapSource = (source) => {
