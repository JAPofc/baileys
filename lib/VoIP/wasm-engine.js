...
export class WasmEngine {
    ...
    initialize = async () => {
        ...
        // JAP@Add (WASM integrity): when an integrity.json manifest sits next
        // to the wasm (written by `npm run voip:fetch-wasm`), verify the
        // binary's sha256 against the pinned hash BEFORE compiling it — a
        // corrupted or tampered whatsapp.wasm fails loudly here instead of
        // executing. Config `skipWasmIntegrityCheck: true` opts out (e.g. for
        // an intentionally patched binary passed via wasmBinary/wasmPath).
        if (!this.#config.skipWasmIntegrityCheck && !this.#config.wasmBinary) {
            const manifestFile = path.join(path.dirname(this.#config.wasmPath), "integrity.json");
            if (fs.existsSync(manifestFile)) {
                try {
                    const manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8"));
                    const expected = manifest?.files?.[path.basename(this.#config.wasmPath)]?.sha256;
                    if (expected) {
                        const actual = createHash("sha256").update(wasmBuffer).digest("hex");
                        if (actual !== expected) {
                            throw new Error(`WASM integrity check failed for ${this.#config.wasmPath}: sha256 ${actual} does not match pinned ${expected}. Re-run \`npm run voip:fetch-wasm\` or pass skipWasmIntegrityCheck: true if the modification is intentional.`);
                        }
                    }
                } catch (err) {
                    if (String(err?.message).includes("integrity check failed")) throw err;
                    // unreadable/corrupt manifest: warn-equivalent no-op, binary still magic-checked below
                }
            }
        }
        // Always sanity-check the wasm magic regardless of manifest presence.
        if (wasmBuffer.length < 8 || wasmBuffer.readUInt32LE(0) !== 0x6d736100) {
            throw new Error(`${this.#config.wasmPath || "wasmBinary"} is not a valid WebAssembly binary (bad magic)`);
        }
