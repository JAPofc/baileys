...
export const prepareRichResponseMessage = (content) => {
    const { alignment, code, contentText, disclaimerText, footerText, headerText, imageText, inlineImage, inlineVideo, items, language, latex, links, noHeading, posts, products, suggested, richResponse, table, tapLinkUrl, text, title } = content;
    // JAP@Fix (rich-message validation): fail fast with actionable messages
    // instead of the confusing TypeErrors these used to produce deep inside.
    if (table !== undefined && table !== null && !Array.isArray(table)) {
        throw new TypeError('rich message: `table` must be an array of rows (each row an array of cell strings)');
    }
    if (links !== undefined && links !== null && !Array.isArray(links)) {
        throw new TypeError('rich message: `links` must be an array of { text, url, ... } objects');
    }
    if (richResponse !== undefined && richResponse !== null && !Array.isArray(richResponse)) {
        throw new TypeError('rich message: `richResponse` must be an array of submessage objects');
    }
    if (code !== undefined && code !== null && typeof code !== 'string') {
        throw new TypeError('rich message: `code` must be a string of source code (set `language` separately)');
    }
    let submessages = [];
    if (Array.isArray(richResponse)) {
        ...
        if (code) {
            // JAP@Fix: `language ||=` reassigned a const destructure — ANY
            // { code } without an explicit language threw
            // "Assignment to constant variable" and the message never sent.
        ...
        if (latex) {
            // JAP@Fix: `text` was never destructured from content — every
            // top-level { latex } message crashed with a ReferenceError.
            submessages.push({
                messageType: RichSubMessageType.LATEX,
                latexMetadata: {
                    text: text ?? contentText ?? '',
                    expressions: Array.isArray(latex) ? latex : [latex]
                }
            });
        }
        if (links) {
