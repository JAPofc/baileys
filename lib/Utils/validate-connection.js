...
const PLATFORM_MAP = {
    ...
    // JAP@Fix (upstream 0af2386, post-rc14): since ~2026-06-30 the server rejects
    // handshakes advertising WIN32 (legacy Electron Desktop identity) — socket
    // closes with 428 ~200-600ms after connect, before any QR is emitted, making
    // Browsers.windows('Desktop') unusable. Modern native Desktop advertises
    // WIN_HYBRID (5). This matters because a Desktop sub-platform is the only way
    // to request full history sync (WEB_BROWSER only gets recent history).
