// Evernight AI — lib/Utils/button-sender.js / Creator : J.AP
// Converted TS -> JS ESM for @japofc/baileys.
//
...
//
// Usage:
//   import { sendButtons, sendInteractiveMessage } from '@japofc/baileys'
//
...
export function convertToInteractiveMessage(content) {
	...
	if (btns && btns.length > 0) {
		...
		// JAP@Fix 30-08-26 --- subtitle was being used only as a fallback for a missing
 ...
 *
 * @param {import('../Types/Socket.js').WASocket} sock Active @japofc/baileys socket instance.
...
export async function sendInteractiveMessageV2(sock, jid, content, options = {}) {
	...
	async function bufferFromUrl(url) {
		// JAP@Fix (dependency matrix): axios was silently required here but never
		// declared anywhere in package.json — without it every URL thumbnail
		// failed. Node >= 18 ships global fetch, so that is now the primary
		// path; axios stays as a fallback for exotic proxy setups that have it.
		try {
