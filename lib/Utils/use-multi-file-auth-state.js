...
export const useMultiFileAuthState = async (folder) => {
    ...
    const writeData = async (data, file) => {
        ...
        return mutex.acquire().then(async (release) => {
            try {
                // JAP@Fix (atomic writes): a crash/power-loss mid-write used to
                // leave a truncated JSON file — for creds.json that means a
                // permanently corrupted session. Write to a temp file first,
                // then rename() (atomic on POSIX within the same directory).
                // Also 0600: auth material must not be world-readable.
                const tmpPath = `${filePath}.tmp-${process.pid}-${Date.now()}`;
                await writeFile(tmpPath, JSON.stringify(data, BufferJSON.replacer), { mode: 0o600 });
    ...
    if (folderInfo) {
        ...
        await mkdir(folder, { recursive: true, mode: 0o700 }); // JAP@Fix: private dir
    }
