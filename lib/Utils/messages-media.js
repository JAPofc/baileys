...
export const extractVideoThumb = async (path, time, size) => {
    // JAP@Add --- auto-detect the binary (ffmpeg-static / @ffmpeg-installer /
    // system PATH / FFMPEG_PATH) instead of blindly assuming PATH
    const { requireFfmpegPath } = await import('./ffmpeg-path.js');
    const ffmpegBin = await requireFfmpegPath();
...
export const generateProfilePicture = async (mediaUpload, dimensions) => {
    let buffer;
    // JAP@Add (full-size profile pictures): `fit: 'contain'` letterboxes the
    // ORIGINAL aspect ratio onto the canvas instead of center-crop — i.e. the
    // whole picture survives, nothing is cut off. Default stays 'cover'
    // (WhatsApp's own crop behavior) so existing callers are unaffected.
    // Usage: sock.updateProfilePicture(jid, img, { fit: 'contain' })
    const { width: w = 720, height: h = 720, fit = 'cover' } = dimensions || {};
    ...
    if ('sharp' in lib && lib.sharp?.default) {
        img = lib.sharp
            .default(buffer)
            .resize(w, h, useContain ? { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 1 } } : undefined)
            .jpeg({
            quality: 80
        })
            .toBuffer();
        ...
        // @napi-rs/image has no letterbox mode — plain resize distorts, so for
        // 'contain' we compute the scaled size manually and accept the smaller
        // canvas (WhatsApp accepts non-square uploads; it renders them fitted).
        if (useContain) {
            ...
            img = new lib.image.Transformer(buffer).resize(sw, sh, 0).jpeg(80);
        }
        else {
        ...
        if (useContain) {
            img = jimp.contain({ w, h }).getBuffer('image/jpeg', { quality: 80 });
        }
        else {
