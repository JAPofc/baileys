...
export const fetchLatestBaileysVersion = async (options = {}) => {
    ...
    try {
        // JAP@Fix: no timeout meant a network blackhole (dropped packets, not
        // refused) hung this fetch — and Bot.start() with it — forever.
        ...
        // JAP@Fix: this used to grab a specific line by index (lines[4]) —
        // extremely fragile, easy to misread if the source file format
        // shifted even slightly (a comment or blank line added/removed above).
        // Now it searches the ENTIRE file for the `const version = [...]`
        // pattern, regardless of which line it's on.
...
export const fetchLatestWaWebVersion = async (options = {}) => {
    try {
        ...
        // JAP@Fix: same hang-forever risk as fetchLatestBaileysVersion — bound it.
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), options.timeoutMs ?? 15_000);
