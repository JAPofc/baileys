...
/**
 * JAP@Add --- music attribution for statuses (the official Dec-2025 WA
 * "add music to your status" surface). Attaches a StatusAttribution of type
 * MUSIC (enum 3) to the status contextInfo — the exact structure official
 * clients put on music statuses (verified against our vendored WAProto:
 * ContextInfo.statusAttributions[] + StatusAttribution.Music round-trip the
 * wire cleanly).
 *
 * HONEST LIMITATION: official clients resolve the song via Meta's licensed
 * music catalog. `songId` must be a real catalog id for artwork/preview to
 * render on official clients; with made-up ids clients show the caption text
 * but may omit the music chip. The attribution itself is wire-correct either
 * way — this is metadata, not the licensed audio stream.
 *
 *   const status = withMusicAttribution(
 *       StatusHelper.image(buffer, 'vibes 🎵'),
 *       { title: 'Song Name', authorName: 'Artist', songId: '<catalog-id>' }
 *   )
 *   await StatusHelper.send(sock, status, jidList)
 */
export const withMusicAttribution = (content, music = {}) => {
    if (!content || typeof content !== 'object') {
        throw new Error('withMusicAttribution: content must be a status content object');
    }
    const { title, authorName, songId, artistAttribution, isExplicit, actionUrl } = music;
    if (!title && !songId) {
        throw new Error('withMusicAttribution: music needs at least { title } or { songId }');
    }
    const attribution = {
        type: 3, // StatusAttribution.Type.MUSIC
        ...(actionUrl ? { actionUrl } : {}),
        music: {
            ...(title ? { title } : {}),
    ...
    return {
        ...
        contextInfo: {
            ...
            statusAttributions: [...(content.contextInfo?.statusAttributions || []), attribution]
        }
    };
};
