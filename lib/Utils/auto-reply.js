...
export class AutoReplyHandler {
    ...
    setCooldown(ruleId, jid, cooldown) {
        ...
        // JAP@Fix (unbounded growth): one entry per ruleId:jid pair was never
        // removed, so a busy public bot leaked memory slowly but forever.
        // Sweep expired entries opportunistically once the map gets large.
        ...
        if (this.globalCooldown.size > 10_000) {
            const now = Date.now();
            const maxAge = this.options.globalCooldown;
            for (const [key, stamp] of this.globalCooldown) {
                if (now - stamp > maxAge) this.globalCooldown.delete(key);
            }
        }
    }
    matchRule(text, rule) {
