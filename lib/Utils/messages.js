...
export const prepareWAMessageMedia = async (message, options) => {
    ...
    if (isNewsletter) {
        ...
        // JAP@Fix (media backend cleanup): the unlink only ran on the success
        // path — every failed newsletter upload leaked its temp file in tmpdir.
        let mediaUrl, directPath, thumbnailDirectPath, thumbnailSha256;
        try {
            ({ mediaUrl, directPath, thumbnailDirectPath, thumbnailSha256 } = await options.upload(filePath, {
            ...
            await fs.unlink(filePath).catch(() => logger?.warn('failed to remove tmp file'));
        }
...
export const generateWAMessageContent = async (message, options) => {
    ...
    // key that prepareRichResponseMessage actually destructures.
        ...
        if (contactLen === 1) {
            // JAP@Fix: the documented shape `{ contacts: { displayName, contacts: [{ vcard }] } }`
            // silently dropped the outer displayName for single contacts — the
            // ContactMessage went out nameless. Fall back to the outer one.
            m.contactMessage = WAProto.Message.ContactMessage.create({
                ...message.contacts.contacts[0],
                displayName: message.contacts.contacts[0].displayName || message.contacts.displayName
            });
        }
        else {
    ...
    // JAP@Changes 31-01-26 --- Add support for album messages
        ...
        // JAP@Fix: protobufjs silently drops unknown fields on encode — a typo'd
        // or unsupported key (e.g. durationSeconds on embeddedMusic) vanished
        // from the wire with zero feedback, making "why doesn't my music render"
        // undebuggable. Validate against the real proto schema up front.
        ...
        if (embeddedMusic && typeof embeddedMusic === 'object') {
            const EMBEDDED_MUSIC_FIELDS = new Set([
                'musicContentMediaId', 'songId', 'author', 'title', 'artworkDirectPath',
                'artworkSha256', 'artworkEncSha256', 'artistAttribution', 'countryBlocklist',
                'isExplicit', 'artworkMediaKey'
            ]);
            const unknown = Object.keys(embeddedMusic).filter((k) => !EMBEDDED_MUSIC_FIELDS.has(k));
            if (unknown.length) {
                throw new Boom(`music.embeddedMusic has unsupported field(s): ${unknown.join(', ')} — ` +
                    `the wire format would silently drop them. Supported: ${[...EMBEDDED_MUSIC_FIELDS].join(', ')}`, { statusCode: 400 });
            }
        }
        m.musicMessage = {
            ...(songUri ? { songUri } : {}),
    ...
    // with older WA versions. groupStatusMessageV2 causes a black screen ("unsupported version" client error)
