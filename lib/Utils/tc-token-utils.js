...
export async function buildTcTokenFromJid({ authState, jid, baseContent = [], getLIDForPN }) {
    try {
        ...
        // JAP@Fix (upstream 731cd6b, post-rc14): a token without a timestamp cannot
        // be advertised — WA Web sends the issue time as the `t` attr, and a missing
        // timestamp means we can't prove freshness, so treat it as unusable.
        ...
        baseContent.push({
            tag: 'tctoken',
            // JAP@Fix (upstream 731cd6b): WA Web sends the token issue time as `t`
