...
const BUFFERABLE_EVENT = [
    ...
    'groups.update',
    // JAP@Fix (bug 43): these events were emitted directly without being buffered,
...
export const makeEventBuffer = (logger, timeoutMs) => {
    ...
    const BUFFER_TIMEOUT_MS = timeoutMs ?? 30000; // JAP@Fix (bug 25): was hardcoded 30s — now configurable via makeWASocket({ eventBufferTimeoutMs })
...
eventData, logger) {
    switch (event) {
        ...
        case 'groups.update':
            ...
            break;
        // JAP@Fix (bug 43): pass-through events — no consolidation needed, just buffer them
