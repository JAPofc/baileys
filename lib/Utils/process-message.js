...
/**
 * JAP@Add --- Encrypt an event response (GOING / NOT_GOING / MAYBE).
 *
 * Exact mirror of `decryptEventResponse` above: same HKDF-style double-HMAC
 * key derivation (info = eventMsgId || creator || responder || "Event
 * Response" || 0x01) and the same AAD (`${eventMsgId}\0${responderJid}`),
 * with a fresh random 12-byte GCM IV. Because the decryptor's constants are
 * validated, an encrypt→decrypt roundtrip proves this construction —
 * see tests/event-response.test.js.
 *
 * @param response `proto.Message.IEventResponseMessage` fields ({ response, timestampMs, extraGuestCount })
 * @param ctx same shape as decryptEventResponse's ctx
 * @returns `{ encPayload, encIv }` ready for `encEventResponseMessage`
 */
export function encryptEventResponse(response, { eventCreatorJid, eventMsgId, eventEncKey, responderJid }) {
    const sign = Buffer.concat([
        toBinary(eventMsgId),
        toBinary(eventCreatorJid),
        toBinary(responderJid),
    ...
    const plaintext = proto.Message.EventResponseMessage.encode(proto.Message.EventResponseMessage.fromObject(response)).finish();
    const encPayload = aesEncryptGCM(plaintext, encKey, encIv, aad);
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
        ...
        // JAP@Fix: stub parameters are NOT always JSON. GROUP_PARTICIPANT_CHANGE_NUMBER
        // stores plain jids (messages-recv 'modify' branch), and history-synced stubs
        // from other clients/old versions do too — JSON.parse on those threw and killed
        // processing of the whole message. Parse JSON when it is JSON, otherwise treat
        // the value as a plain jid.
        const parseStubParticipant = (a) => {
            if (typeof a !== 'string') return a;
            try {
                const v = JSON.parse(a);
                return typeof v === 'object' && v !== null ? v : { phoneNumber: String(v) };
            }
            catch {
        ...
        switch (message.messageStubType) {
            case WAMessageStubType.GROUP_PARTICIPANT_CHANGE_NUMBER:
                participants = (message.messageStubParameters || []).map(parseStubParticipant);
            ...
            case WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_REQUEST_NON_ADMIN_ADD: // TODO: Add other events
                const participant = parseStubParticipant(message.messageStubParameters?.[0]);
