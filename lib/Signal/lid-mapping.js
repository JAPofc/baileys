...
export class LIDMappingStore {
    ...
    async _getPNsForLIDsImpl(lids) {
        ...
        for (const lid of lids) {
            // JAP@Fix (hosted LID): isLidUser() only matches @lid, so hosted
            // companion jids (@hosted.lid) were silently skipped here — the
            // reverse LID->PN resolution never happened for hosted devices
            // even though the forward path (PN->LID) fully supports them.
