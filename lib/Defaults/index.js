...
// JAP@Fix: aligned with the baseline used for the WAProto diff (see
// WAProto/CHANGELOG-proto-upgrade.md — it was newer than the old fallback,
// so the socket claimed an older version than the fields its own proto
// already supported). This is only a fallback for when fetchLatestBaileysVersion()
// fails completely (e.g. GitHub unreachable) — still worth updating
// periodically, and at runtime prefer fetchLatestWaWebVersion() first
// (parses sw.js directly), then fall back to fetchLatestBaileysVersion()
// / this constant, since Baileys' official fetchLatestBaileysVersion has a
// history of reporting isLatest:true for stale versions (upstream#2679).
const version = [2, 3000, 1048351027]; // JAP@Update: live sw.js client_revision as of 2026-09-22 (auto-bumped weekly by version-watchdog)
